...
/**
 * Created by     : open9527
 * Created times  : on 2019/9/12 12:02.
...
//        CustomViewUtils.showLogInfo(TAG,"position+++++" + mCurSelectedPosition);
        detachAndScrapAttachedViews(recycler);
...
//        CustomViewUtils.showLogInfo(TAG,"firstFillCover finish:first: " + mFirstVisiblePosition + ",last:" + mLastVisiblePos);
...
//        CustomViewUtils.showLogInfo(TAG,"calculateToCenterFraction: distance:" + distance + ",childLength:" + childLength);
...
//        CustomViewUtils.showLogInfo(TAG,"fillWithVertical: dy:" + dy);
        int topEdge = getOrientationHelper().getStartAfterPadding();
        int bottomEdge = getOrientationHelper().getEndAfterPadding();
...
//                        CustomViewUtils.showLogInfo(TAG,"fillWithVertical: removeAndRecycleView:" + getPosition(child) + ",bottom:" + getDecoratedBottom(child));
                        removeAndRecycleView(child, recycler);
                        mFirstVisiblePosition++;
                        fixIndex--;
                    } else {
//                        CustomViewUtils.showLogInfo(TAG,"fillWithVertical: break:" + getPosition(child) + ",bottom:" + getDecoratedBottom(child));
                        break;
                    }
                }
            } else { //dy<0
...
//                        CustomViewUtils.showLogInfo(TAG, "fillWithVertical: removeAndRecycleView:" + getPosition(child));
                        removeAndRecycleView(child, recycler);
                        mLastVisiblePos--;
                    } else {
                        break;
        ...
        if (dy >= 0) {
            ...
            for (int i = startPosition; i < getItemCount() && startOffset < bottomEdge + dy; i++) {
                ...
                startOffset = scrapRect.bottom;
                mLastVisiblePos = i;
//                CustomViewUtils.showLogInfo(TAG,"fillWithVertical: add view:" + i + ",startOffset:" + startOffset + ",mLastVisiblePos:" + mLastVisiblePos + ",bottomEdge" + bottomEdge);
            }
        } else {
            //dy<0
            if (getChildCount() > 0) {
    ...
    private void fillWithHorizontal(RecyclerView.Recycler recycler, RecyclerView.State state, int dx) {
        int leftEdge = getOrientationHelper().getStartAfterPadding();
        int rightEdge = getOrientationHelper().getEndAfterPadding();
//        CustomViewUtils.showLogInfo(TAG,"fillWithHorizontal() called with: dx = [" + dx + "],leftEdge:" + leftEdge + ",rightEdge:" + rightEdge);
        //1.remove and recycle the view that disappear in screen
        View child;
        if (getChildCount() > 0) {
            if (dx >= 0) {
                ...
                for (int i = 0; i < getChildCount(); i++) {
                    ...
                    if (getDecoratedRight(child) - dx < leftEdge) {
                        ...
                        mFirstVisiblePosition++;
                        fixIndex--;
//                        CustomViewUtils.showLogInfo(TAG,"fillWithHorizontal:removeAndRecycleView:" + getPosition(child) + " mFirstVisiblePosition change to:" + mFirstVisiblePosition);
                    } else {
                        break;
                    }
                }
                ...
                for (int i = getChildCount() - 1; i >= 0; i--) {
                    ...
                    if (getDecoratedLeft(child) - dx > rightEdge) {
                        removeAndRecycleView(child, recycler);
                        mLastVisiblePos--;
//                        CustomViewUtils.showLogInfo(TAG,"fillWithHorizontal:removeAndRecycleView:" + getPosition(child) + "mLastVisiblePos change to:" + mLastVisiblePos);
                    }
                }
            }
...
//                CustomViewUtils.showLogInfo(TAG,"fillWithHorizontal:to right startPosition:" + startPosition + ",startOffset:" + startOffset + ",rightEdge:" + rightEdge);
            }
            for (int i = startPosition; i < getItemCount() && startOffset < rightEdge + dx; i++) {
                ...
                startOffset = scrapRect.right;
                mLastVisiblePos = i;
//                CustomViewUtils.showLogInfo(TAG,"fillWithHorizontal,layout:mLastVisiblePos: " + mLastVisiblePos);
            }
        } else {
            //dx<0
            if (getChildCount() > 0) {
...
//                CustomViewUtils.showLogInfo(TAG,"fillWithHorizontal:to left startPosition:" + startPosition + ",startOffset:" + startOffset + ",leftEdge:" + leftEdge + ",child count:" + getChildCount());
            }
...
//        CustomViewUtils.showLogInfo(TAG,"scrollHorizontallyBy: dx:" + dx + ",fixed:" + delta);
...
//        CustomViewUtils.showLogInfo(TAG,"scrollVerticallyBy: dy:" + dy + ",fixed:" + delta);
        getState().mScrollDelta = -delta;
        fillCover(recycler, state, -delta);
...
//        CustomViewUtils.showLogInfo("","position-----"+mCurSelectedPosition);
